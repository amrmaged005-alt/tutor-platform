...
export async function POST(
  ...
  try {
    ...
    // Fire-and-forget: send notification email (best-effort, non-blocking)
    if (user.email) {
      const baseUrl = process.env.NEXT_PUBLIC_BASE_URL ?? "https://coursaty.com";
      sendWaitlistNotification({
        to: user.email,
        firstName: user.fullName ?? user.name ?? "Student",
        classTitle: cls.title,
        tutorName: "",
        priceEgp: 0,
        classUrl: `${baseUrl}/classes/${id}`,
      }).catch(() => null);
    }
    ...
    console.error("[POST /api/classes/[id]/waitlist] error:", err);
...
export async function DELETE(
  ...
  try {
    ...
    console.error("[DELETE /api/classes/[id]/waitlist] error:", err);
    return NextResponse.json({ error: "Internal server error" }, { status: 500 });
  }
}
