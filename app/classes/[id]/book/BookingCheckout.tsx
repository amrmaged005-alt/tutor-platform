...
export default function BookingCheckout({ cls }: { cls: ClassSummary }) {
  ...
  const handleBook = useCallback(async () => {
    ...
    try {
      const response = await fetch("/api/bookings", {
        ...
        body: JSON.stringify({ classId: cls.id, paymentType: cls.paymentType, ...(promoCode ? { promoCode } : {}) }),
      });
  ...
  }, [cls.id, cls.paymentType, promoCode, submitting]);
  ...
  return (
    <main style={{ maxWidth: 860, margin: "0 auto", padding: "1rem 1rem 5rem", color: "var(--text)" }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 12, marginBottom: 18 }}>
        ...
        <button type="button" onClick={() => setLang(lang === "en" ? "ar" : "en")} className="btn-secondary" style={{ minHeight: 34, padding: "6px 10px", fontSize: 11 }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "minmax(0, 1fr) minmax(250px, 280px)", gap: 14, alignItems: "start" }} className="checkout-grid">
        <section style={{ minHeight: 330, padding: "1.25rem", background: "var(--bg-card)", border: "1px solid var(--border-light)", borderRadius: "var(--radius-lg)" }}>
          <AnimatePresence mode="wait">
            <motion.div key={step} initial={reduceMotion ? false : { opacity: 0, x: 10 }} animate={{ opacity: 1, x: 0 }} exit={reduceMotion ? undefined : { opacity: 0, x: -10 }} transition={{ duration: 0.2 }}>
          ...
          <div style={{ marginTop: 22 }}>
            {step < STEPS.length - 1 ? (
              <div style={{ display: "flex", justifyContent: "space-between", gap: 10 }}>
                <button type="button" onClick={() => setStep((current) => Math.max(0, current - 1))} disabled={step === 0 || submitting} className="btn-secondary">{t("booking.checkout.back")}</button>
                <button type="button" onClick={() => setStep((current) => Math.min(STEPS.length - 1, current + 1))} className="btn-primary" style={{ flex: 1 }}>
                  {t("booking.checkout.continue")} <ChevronRight size={15} aria-hidden />
                </button>
              </div>
            ) : (
              <div>
...
function StepIndicator({ step, t }: { step: number; t: Translator }) {
  return (
    <ol style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", margin: "0 0 22px", padding: 0, listStyle: "none" }} aria-label="Booking steps">
  ...
  return (
    <>
      ...
      <div style={{ display: "grid", gap: 12 }}>
        {[t("booking.checkout.nextDashboard"), t("booking.checkout.nextTutor", { name: cls.tutorName }), t("booking.checkout.nextMessages")].map((item) => (
...
function PaymentStep({ cls, promoResult, onApply, onClear, disabled, t }: { cls: ClassSummary; promoResult: PromoResult | null; onApply: (code: string, result: PromoResult) => void; onClear: () => void; disabled: boolean; t: Translator }) {
  return (
    <>
      <h2 style={{ margin: "0 0 6px", fontSize: "1.1rem" }}>{t("booking.checkout.paymentTitle")}</h2>
      <p style={{ margin: "0 0 18px", color: "var(--text-secondary)", fontSize: 14 }}>{t(cls.paymentType === "ONLINE" ? "booking.checkout.paymentOnline" : "booking.checkout.paymentCash")}</p>
      <PromoCodeInput classId={cls.id} priceEgp={cls.priceEgp} onApply={onApply} onClear={onClear} disabled={disabled} />
      {promoResult && <p style={{ margin: "12px 0 0", color: "var(--success)", fontSize: 13, fontWeight: 700 }}>{t("booking.checkout.promoApplied", { amount: promoResult.discountEgp })}</p>}
    </>
  );
}

function OrderSummary({ cls, promoResult, finalPrice, t }: { cls: ClassSummary; promoResult: PromoResult | null; finalPrice: number; t: Translator }) {
  return (
    <aside style={{ position: "sticky", top: 84, padding: "1.25rem", background: "var(--bg-card)", border: "1px solid var(--border-light)", borderRadius: "var(--radius-lg)" }}>
      ...
      <div style={{ display: "grid", gap: 8, paddingTop: 12, borderTop: "1px solid var(--border-light)" }}>
        ...
        {promoResult && <PriceRow label={t("booking.checkout.discount", { amount: promoResult.discountPct })} value={`-${promoResult.discountEgp} EGP`} highlight />}
