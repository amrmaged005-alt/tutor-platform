...
export default function ClassSearch({ initialClasses }: { initialClasses: ClassResult[] }) {
  ...
  return (
    <div>
      {/* Search bar row */}
      <div style={{ maxWidth: 900, margin: "0 auto 1.5rem", padding: "0 2rem", display: "flex", gap: "0.75rem", alignItems: "center", flexWrap: "wrap" as const }}>
        <input
          type="text"
          placeholder="Search classes, tutors, centers..."
          value={search}
          onChange={e => setSearch(e.target.value)}
          style={{ flex: 1, minWidth: 220, backgroundColor: "#1e293b", color: "#f1f5f9", border: "1px solid #334155", borderRadius: 10, padding: "0.65rem 1rem", fontSize: 15, outline: "none" }}
        />
        <button
          onClick={() => setFiltersOpen(o => !o)}
          style={{ backgroundColor: filtersOpen ? "#3b82f6" : "#1e293b", color: filtersOpen ? "white" : "#94a3b8", border: "1px solid #334155", borderRadius: 10, padding: "0.65rem 1.25rem", fontSize: 14, cursor: "pointer", fontWeight: 600 }}
        >
          {filtersOpen ? "Hide filters" : "Filters"}
          {hasFilters && !filtersOpen && (
            <span style={{ marginLeft: 6, backgroundColor: "#3b82f6", color: "white", borderRadius: 20, fontSize: 11, padding: "1px 7px" }}>ON</span>
          )}
        </button>
      ...
      {/* Filters panel */}
      {filtersOpen && (
        <div style={{ maxWidth: 900, margin: "0 auto 2rem", padding: "1.25rem 1.5rem", backgroundColor: "#1e293b", border: "1px solid #334155", borderRadius: 14, marginLeft: "2rem", marginRight: "2rem" }}>
          <div style={{ display: "flex", flexWrap: "wrap" as const, gap: "1rem", alignItems: "flex-end" }}>
            ...
            <div>
              ...
              <div style={{ display: "flex", gap: 6 }}>
                <input type="number" placeholder="Min" value={minPrice} onChange={e => setMinPrice(e.target.value)} style={{ ...selectStyle, width: 70 }} />
                <input type="number" placeholder="Max" value={maxPrice} onChange={e => setMaxPrice(e.target.value)} style={{ ...selectStyle, width: 70 }} />
              </div>
            </div>

            {/* NEW: Location filter */}
            <div>
              <label style={{ display: "block", fontSize: 11, color: "#64748b", marginBottom: 4, fontWeight: 600, textTransform: "uppercase" as const }}>Location</label>
              <input
                type="text"
            ...
            {hasFilters && (
              <button onClick={clearAll} style={{ backgroundColor: "transparent", color: "#f87171", border: "1px solid #f87171", borderRadius: 8, padding: "0.5rem 1rem", fontSize: 13, cursor: "pointer" }}>
                Clear all
              </button>
            )}
          </div>
      ...
      {/* Class grid */}
      <div style={{ maxWidth: 1100, margin: "0 auto", padding: "0 2rem 4rem", display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(320px, 1fr))", gap: "1.5rem", opacity: loading ? 0.5 : 1, transition: "opacity 0.2s" }}>
        {classes.length === 0 && !loading && (
          <div style={{ gridColumn: "1/-1", textAlign: "center" as const, color: "#64748b", padding: "3rem" }}>
        ...
        {classes.map((cls) => {
          ...
          const displayName = cls.center ? cls.center.name : cls.owner?.fullName ?? "Unknown";
          const isCenter = !!cls.center;

          return (
            <Link key={cls.id} href={"/classes/" + cls.id} style={{ textDecoration: "none" }}>
              <div style={{ backgroundColor: "#1e293b", border: "1px solid #334155", borderRadius: 16, padding: "1.5rem", height: "100%", boxSizing: "border-box" as const, transition: "border-color 0.2s" }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: "1rem" }}>
                  <span style={{ backgroundColor: style.bg, color: style.text, fontSize: 12, fontWeight: 700, padding: "4px 12px", borderRadius: 20 }}>
                    {style.emoji} {cls.subject}
                  </span>
                  <div style={{ display: "flex", gap: 6 }}>
                    <span style={{ backgroundColor: cls.format === "ONLINE" ? "#0c4a6e" : cls.format === "HYBRID" ? "#1e3a5f" : "#0f172a", color: cls.format === "ONLINE" ? "#38bdf8" : "#94a3b8", fontSize: 10, fontWeight: 600, padding: "3px 8px", borderRadius: 20, border: "1px solid #334155" }}>
                      {formatLabel[cls.format] ?? cls.format}
                    </span>
                    {isFull && <span style={{ backgroundColor: "#450a0a", color: "#fca5a5", fontSize: 11, fontWeight: 600, padding: "3px 8px", borderRadius: 20 }}>FULL</span>}
                    {!isFull && spotsLeft !== null && spotsLeft <= 3 && <span style={{ backgroundColor: "#451a03", color: "#fdba74", fontSize: 11, fontWeight: 600, padding: "3px 8px", borderRadius: 20 }}>Only {spotsLeft} left</span>}
                  </div>
                </div>

                <h2 style={{ fontSize: "1.05rem", fontWeight: 700, color: "#f1f5f9", marginBottom: "0.4rem" }}>{cls.title}</h2>
